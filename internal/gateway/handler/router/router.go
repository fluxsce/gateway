...
func (r *Router) Handle(ctx *core.Context) bool {
	...
	// 检查全局前置过滤器并保存原始信息
	hasGlobalPreFilters := reqhand.PreserveOriginalRequestInfoIfNeeded(ctx, r.routerFilters)
	...
	if !hasGlobalPreFilters {
		reqhand.PreserveOriginalRequestInfoIfNeeded(ctx, route.GetRouteFilters())
	}

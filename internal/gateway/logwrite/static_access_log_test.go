...
func TestShouldSkipStaticAccessLog(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "redirect", proxy: "static", result: "redirect", status: http.StatusMovedPermanently, setStatus: true, want: true},
		...
		{name: "server error", proxy: "static", result: "forbidden", status: http.StatusInternalServerError, setStatus: true, want: false},

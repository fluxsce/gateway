...
func TestOracleTransaction(t *testing.T) {
	...
	err = db.InTx(ctx, nil, func() error {
		_, err := db.Exec(ctx, "INSERT INTO test_tx_users (id, name) VALUES (1, '事务测试1')", nil, false)
		if err != nil {
			return err
		}
		_, err = db.Exec(ctx, "INSERT INTO test_tx_users (id, name) VALUES (2, '事务测试2')", nil, false)
		return err
	})
	...
	// 测试事务回滚
	err = db.InTx(ctx, nil, func() error {
		_, err := db.Exec(ctx, "INSERT INTO test_tx_users (id, name) VALUES (3, '事务测试3')", nil, false)
		if err != nil {
			return err
		}
		// 故意返回错误，触发回滚

...
func TestSQLiteTransaction(t *testing.T) {
	...
	err = db.InTx(ctx, nil, func(txCtx context.Context) error {
		_, err := db.Exec(txCtx, "INSERT INTO test_tx (name) VALUES (?)", []interface{}{"test1"}, false)
		if err != nil {
			return err
		}
		_, err = db.Exec(txCtx, "INSERT INTO test_tx (name) VALUES (?)", []interface{}{"test2"}, false)
		return err
	})
	if err != nil {
	...
	// 测试事务回滚
	err = db.InTx(ctx, nil, func(txCtx context.Context) error {
		_, err := db.Exec(txCtx, "INSERT INTO test_tx (name) VALUES (?)", []interface{}{"test3"}, false)
		if err != nil {
			return err
		}
		// 模拟错误导致回滚

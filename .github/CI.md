...
- 推送 tag：`v*`（例如 `v3.3.9`），版本号去掉 `v`
- 手动：Actions → **Release packages** → `version`（如 `3.3.9`），可选是否推镜像
...
CI 用 `GITHUB_TOKEN` 推送（以 `3.3.9` 为例）：

```text
ghcr.io/fluxsce/gateway:3.3.9
ghcr.io/fluxsce/gateway:3.3.9-amd64
ghcr.io/fluxsce/gateway:3.3.9-arm64
ghcr.io/fluxsce/gateway:3.3.9-oracle
# 仅 git tag 发布时还有：
...
# 按本机架构自动选层
docker pull ghcr.io/fluxsce/gateway:3.3.9

# 指定架构
docker pull ghcr.io/fluxsce/gateway:3.3.9-arm64
docker pull ghcr.io/fluxsce/gateway:3.3.9-amd64

# Oracle（仅 amd64）
docker pull ghcr.io/fluxsce/gateway:3.3.9-oracle
```
...
CI 在 Secrets 配好后推送到（以 `3.3.9` 为例）：

```text
crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com/datahub-images/gateway:3.3.9
crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com/datahub-images/gateway:3.3.9-amd64
crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com/datahub-images/gateway:3.3.9-arm64
crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com/datahub-images/gateway:3.3.9-oracle
# 仅 git tag 发布时还有：
...
# 按本机架构自动选层
docker pull crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com/datahub-images/gateway:3.3.9

# 指定架构
docker pull crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com/datahub-images/gateway:3.3.9-arm64
docker pull crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com/datahub-images/gateway:3.3.9-amd64

# Oracle（仅 amd64）
docker pull crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com/datahub-images/gateway:3.3.9-oracle
```

...
Examples use **3.3.6**. Prefer the latest assets on [GitHub Releases](https://github.com/fluxsce/gateway/releases).
...
| Listen health | http://localhost:8080/_gw/health (after the instance binds) |
...
|------|----------|-----------|
| `gateway-linux-amd64-3.3.6.tar.gz` | Linux amd64 | MySQL / SQLite / ClickHouse |
| `gateway-linux-arm64-3.3.6.tar.gz` | Linux arm64 | same |
| `gateway-windows-amd64-3.3.6.zip` | Windows amd64 | same |
| `gateway-linux-amd64-oracle-3.3.6.tar.gz` | Linux amd64 | above + Oracle |
| `gateway-windows-amd64-oracle-3.3.6.zip` | Windows amd64 | above + Oracle |

```bash
tar -xzf gateway-linux-amd64-3.3.6.tar.gz
cd gateway

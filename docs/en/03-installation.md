...
Examples use **3.2.9** and `/opt/gateway`.
...
```bash
wget https://github.com/fluxsce/gateway/releases/download/v3.2.9/gateway-linux-amd64-3.2.9.tar.gz
sudo tar -xzf gateway-linux-amd64-3.2.9.tar.gz -C /opt
cd /opt/gateway
```

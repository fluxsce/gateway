...
Examples use **3.3.7** and `/opt/gateway`.
...
```bash
wget https://github.com/fluxsce/gateway/releases/download/v3.3.7/gateway-linux-amd64-3.3.7.tar.gz
sudo tar -xzf gateway-linux-amd64-3.3.7.tar.gz -C /opt
cd /opt/gateway
```

...
Examples use **3.3.8** and `/opt/gateway`.
...
```bash
wget https://github.com/fluxsce/gateway/releases/download/v3.3.8/gateway-linux-amd64-3.3.8.tar.gz
sudo tar -xzf gateway-linux-amd64-3.3.8.tar.gz -C /opt
cd /opt/gateway
```

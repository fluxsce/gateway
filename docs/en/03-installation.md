...
Examples use **3.3.0** and `/opt/gateway`.
...
```bash
wget https://github.com/fluxsce/gateway/releases/download/v3.3.0/gateway-linux-amd64-3.3.0.tar.gz
sudo tar -xzf gateway-linux-amd64-3.3.0.tar.gz -C /opt
cd /opt/gateway
```

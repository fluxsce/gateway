...
Examples use **3.3.9** and `/opt/gateway`.
...
```bash
wget https://github.com/fluxsce/gateway/releases/download/v3.3.9/gateway-linux-amd64-3.3.9.tar.gz
sudo tar -xzf gateway-linux-amd64-3.3.9.tar.gz -C /opt
cd /opt/gateway
```

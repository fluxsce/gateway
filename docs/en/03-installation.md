...
Examples use **3.4.3** and `/opt/gateway`.
...
```bash
wget https://github.com/fluxsce/gateway/releases/download/v3.4.3/gateway-linux-amd64-3.4.3.tar.gz
sudo tar -xzf gateway-linux-amd64-3.4.3.tar.gz -C /opt
cd /opt/gateway
```

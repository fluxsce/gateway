...
Examples use **3.4.2** and `/opt/gateway`.
...
```bash
wget https://github.com/fluxsce/gateway/releases/download/v3.4.2/gateway-linux-amd64-3.4.2.tar.gz
sudo tar -xzf gateway-linux-amd64-3.4.2.tar.gz -C /opt
cd /opt/gateway
```

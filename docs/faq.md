...
安装包与镜像版本以 [GitHub Releases](https://github.com/fluxsce/gateway/releases) 为准。下文以 3.2.5 为例。
...
scripts\build\build-win10.cmd --version=3.2.5

# 需要 Oracle 时
scripts\build\build-win10.cmd --oracle --version=3.2.5

# Linux（默认 no_oracle）
./scripts/build/build-centos7.sh --version=3.2.5
./scripts/build/build-centos7.sh --oracle --version=3.2.5
```

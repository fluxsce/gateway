@echo off
:: 设置代码页为UTF-8（Windows Server 2008需要检查支持情况）
chcp 65001 2>nul || chcp 936
setlocal EnableDelayedExpansion

:: 设置错误处理 - 即使有错误也不退出
set ORIGINAL_ERRORLEVEL=%errorlevel%

:: GoHub Windows Server 2008 服务安装脚本
:: 用法: install-service-win2008.cmd [options]

title GoHub Windows 服务安装 - Windows Server 2008

:: 显示调试信息
echo.
echo ==========================================
echo  GoHub Windows 服务安装调试信息
echo  Windows Server 2008 兼容版本
echo ==========================================
echo.
echo [DEBUG] 脚本路径: %~dp0
echo [DEBUG] 脚本名称: %~nx0
...
echo [DEBUG] 操作系统: %OS%
echo [DEBUG] 处理器架构: %PROCESSOR_ARCHITECTURE%
echo.

:: 检查Windows版本
echo [INFO] 检查Windows版本...
ver | findstr /i "6.0" >nul
if %errorlevel% equ 0 (
    echo [INFO] 检测到Windows Server 2008/Vista
    set WIN2008_MODE=true
) else (
    echo [INFO] 检测到其他Windows版本
    set WIN2008_MODE=false
)
echo.
...
:show_help
echo.
echo GoHub Windows Server 2008 服务安装脚本
echo.
...
echo.
echo 注意:
echo   - 此版本专为Windows Server 2008优化
...
echo 日志目录: %LOG_DIR%
echo Windows Server 2008 模式: %WIN2008_MODE%
echo.
...
:: 设置服务描述（检查Windows Server 2008兼容性）
echo [DEBUG] 设置服务描述...
sc description "%SERVICE_NAME%" "%SERVICE_DESCRIPTION%" >nul 2>&1
if %errorlevel% neq 0 (
    echo [DEBUG] 服务描述设置失败，错误码: %errorlevel%
    echo [DEBUG] 在Windows Server 2008上，某些sc命令可能不完全支持
) else (
    echo [DEBUG] ✓ 服务描述设置成功
)

:: 设置服务恢复选项（检查Windows Server 2008兼容性）
echo [DEBUG] 设置服务恢复选项...
sc failure "%SERVICE_NAME%" reset= 86400 actions= restart/5000/restart/5000/restart/5000 >nul 2>&1
if %errorlevel% neq 0 (
    echo [DEBUG] 服务恢复选项设置失败，错误码: %errorlevel%
    echo [DEBUG] 在Windows Server 2008上，failure命令可能不完全支持
    echo [DEBUG] 建议手动通过服务管理器设置恢复选项
) else (
    echo [DEBUG] ✓ 服务恢复选项设置成功
)
...
if %errorlevel% neq 0 (
    echo [ERROR] 服务注册表项不存在，服务创建可能失败
    echo [DEBUG] 请检查服务是否正确创建
) else (
    echo [DEBUG] ✓ 服务注册表项存在
)
...
if %START_RESULT% equ 0 (
    ...
    echo   4. 数据库连接失败
    echo   5. 权限不足
    echo   6. Windows Server 2008 兼容性问题
    echo.
    echo [DEBUG] 建议检查Windows事件日志获取更多信息
)
...
echo [DEBUG] 验证环境变量...
if "%DEBUG_MODE%"=="false" (
    reg query "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\%SERVICE_NAME%\Environment" /v GOHUB_CONFIG_DIR 2>nul | findstr "GOHUB_CONFIG_DIR"
    if %errorlevel% neq 0 (
        echo [DEBUG] 环境变量未设置或查询失败
    )
) else (
...
echo   - Windows事件日志（事件查看器 -> Windows日志 -> 应用程序）
echo   - 应用程序日志目录：%LOG_DIR%
echo.
echo [INFO] Windows Server 2008 特别说明：
echo   - 某些高级服务配置功能可能不完全支持
echo   - 建议使用services.msc进行图形化管理
echo   - 如遇到问题，请检查事件日志获取详细信息
echo.
echo [INFO] 按任意键退出...
pause
exit /b 0

:: Windows Server 2008 兼容的等待函数
:wait_seconds
set /a wait_time=%1
echo [DEBUG] 等待 %wait_time% 秒...
:: 在Windows Server 2008上，timeout命令可能不可用，使用ping作为替代
ping 127.0.0.1 -n %wait_time% >nul 2>&1
exit /b 0
...
if exist "%CANDIDATE_DIR%" (
    dir /b "%CANDIDATE_DIR%\*.exe" 2>nul | findstr /i "gohub"
    if %errorlevel% neq 0 echo   - 未找到GoHub相关exe文件
) else (
    echo   - 目录不存在
)
echo.
...
if exist "%PROJECT_DIR%" (
    dir /b "%PROJECT_DIR%\*.exe" 2>nul | findstr /i "gohub"
    if %errorlevel% neq 0 echo   - 未找到GoHub相关exe文件
) else (
    echo   - 目录不存在
)
...
if exist "%SCRIPT_DIR%" (
    dir /b "%SCRIPT_DIR%\*.exe" 2>nul | findstr /i "gohub"
    if %errorlevel% neq 0 echo   - 未找到GoHub相关exe文件
) else (
    echo   - 目录不存在
)
...
:: 优先检查Windows Server 2008专用版本
if exist "%CHECK_DIR%\gohub-win2008-amd64.exe" (
    set EXE_FILE=%CHECK_DIR%\gohub-win2008-amd64.exe
    echo [INFO] 找到Windows Server 2008专用版本
    exit /b 0
...
:: 没找到相关文件
echo [DEBUG] 在目录 %CHECK_DIR% 中未找到可执行文件
exit /b 1 
CREATE TABLE HUB_ALERT_TEMPLATE (
  ...
  -- 模板基本信息
  templateDesc VARCHAR2(500), -- 模板描述
  channelType VARCHAR2(50), -- 适用的渠道类型：email/qq/wechat_work/dingtalk/webhook/sms，为空表示通用模板
  
  -- 模板内容
  titleTemplate VARCHAR2(500), -- 标题模板，支持变量占位符如{{.Title}}
  contentTemplate CLOB, -- 内容模板，支持变量占位符
  displayFormat VARCHAR2(20) DEFAULT 'table' NOT NULL, -- 显示格式：table表格格式/text文本格式
  templateVariables CLOB, -- 模板变量定义，JSON格式，描述可用的变量和说明
  
  -- 附件配置
  attachmentConfig CLOB, -- 附件配置，JSON格式，用于邮件附件等

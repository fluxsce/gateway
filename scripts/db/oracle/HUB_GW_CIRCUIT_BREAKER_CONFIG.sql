CREATE TABLE HUB_GW_CIRCUIT_BREAKER_CONFIG (
    ...
    -- 运行时固定按节点摘除，本列 NOT NULL，写入 node
                                                    keyStrategy     VARCHAR2(50) DEFAULT 'node' NOT NULL,
    ...
    -- 阈值配置
                                                    ...
                                                    slowCallThreshold     NUMBER(10) DEFAULT 60000 NOT NULL, -- 单次转发超过该毫秒数计为慢调用
                                                    slowCallRatePercent   NUMBER(10) DEFAULT 50 NOT NULL, -- 慢调用率阈值(百分比)

    -- 时间配置
                                                    openTimeoutSeconds    NUMBER(10) DEFAULT 60 NOT NULL, -- 熔断器打开持续时间(秒)
                                                    windowSizeSeconds     NUMBER(10) DEFAULT 60 NOT NULL, -- 滑动窗口长度(秒)

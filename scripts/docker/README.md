...
docker build \
    ...
    -t datahub-images/gateway:3.2.5 .
```

### 使用构建脚本

```bash
# 构建 Oracle 版（默认），标签 :3.2.5-oracle
...
docker build \
    ...
    --build-arg VERSION=3.2.5 \
    --build-arg BUILD_DATE=$(date -u +'%Y-%m-%dT%H:%M:%SZ') \
    --build-arg GIT_COMMIT=$(git rev-parse --short HEAD 2>/dev/null || echo "unknown") \
    -t datahub-images/gateway:3.2.5-oracle .
...
# 推送 Oracle 版到阿里云（默认），远程 :3.2.5-oracle
...
# 推送标准版
docker push datahub-images/gateway:3.2.5

# 推送 Oracle 版
docker push datahub-images/gateway:3.2.5-oracle

# 登录阿里云镜像仓库
docker login crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com

# 标记镜像
docker tag datahub-images/gateway:3.2.5 \
  crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com/datahub-images/gateway:3.2.5
docker tag datahub-images/gateway:3.2.5-oracle \
  crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com/datahub-images/gateway:3.2.5-oracle

# 推送到阿里云
docker push crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com/datahub-images/gateway:3.2.5
```
...
- **标签**: `{version}-oracle`（如 `3.2.5-oracle`）

...
docker build \
    ...
    -t datahub-images/gateway:3.0.2 .
```
...
# 推送到 Docker Hub
docker push datahub-images/gateway:3.0.2

# 登录阿里云镜像仓库
docker login crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com

# 标记镜像
docker tag datahub-images/gateway:3.0.2 \
  crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com/datahub-images/gateway:3.0.2

# 推送到阿里云
docker push crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com/datahub-images/gateway:3.0.2
```
...
- **标签**:
  - `3.0.2` - 默认版本（包含所有依赖：MySQL/SQLite/Oracle）
...
#### Gateway 服务
- **镜像**: datahub-images/gateway:3.0.2
- **端口**:

...
docker build \
    ...
    -t datahub-images/gateway:3.3.0 .
```

### 使用构建脚本

```bash
# 构建 Oracle 版（默认），标签 :3.3.0-oracle
...
docker build \
    ...
    -t datahub-images/gateway:3.3.0-oracle .
...
# 推送 Oracle 版到阿里云（默认），远程 :3.3.0-oracle
...
# 推送标准版
docker push datahub-images/gateway:3.3.0

# 推送 Oracle 版
docker push datahub-images/gateway:3.3.0-oracle

# 登录阿里云镜像仓库
docker login crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com

# 标记镜像
docker tag datahub-images/gateway:3.3.0 \
  crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com/datahub-images/gateway:3.3.0
docker tag datahub-images/gateway:3.3.0-oracle \
  crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com/datahub-images/gateway:3.3.0-oracle

# 推送到阿里云
docker push crpi-25xt72cd1prwdj5s.cn-hangzhou.personal.cr.aliyuncs.com/datahub-images/gateway:3.3.0
```
...
- **标签**: `{version}-oracle`（如 `3.3.0-oracle`）
...
标签：`3.3.0`（标准版，多架构清单）、`3.3.0-oracle`（仅 amd64）、`latest`（仅 git tag 发版）。
...
#### Gateway 服务
- **镜像**: `ghcr.io/fluxsce/gateway:3.3.0`（compose 默认；国内可改为 ACR）
- **端口**:

...
### 修复
- **Mongo 日志库连不上不再挡住网关启动**：`mongo.enabled` 且某条连接校验失败、地址不通或认证失败时，记错误并跳过该连接，进程继续听端口。单次拨号不超过 5 秒，配置里的选主超时更短时用配置。失败的连接每 30 秒重试一次，连上后访问日志写入器和索引对齐会接着用默认连接。配置文件打不开或 `mongo` 段无法解析时仍只记错误，不退出进程。

## [3.4.4] - 2026-09-24

### 新增
- **网关实例右键可看集群端口**：在某一行上右键，只探测这个实例在各存活节点上的监听口。窗口按节点画出探活结果，并显示节点 ID，不用实例表里会被覆盖的健康标记。公用接口只收模块自己的请求参数，按钮也按该模块单独校验。其它监听页登记自己的按钮和端口查询后即可挂上。

### 变更
- **集群节点自行登记**：去掉 `app.cluster.node_id`。集群和采集共用 `GetNodeId()`。进程启动后写入 `HUB_CLUSTER_NODE` 并刷新心跳，退出时删除自己的行。心跳过期的节点会从集群表和采集节点表清掉，曲线历史仍按保留天数清理。
- **列表每页条数改由 Web 访问设置控制**：环境设置「Web 访问」增加默认每页条数（缺省 20）和每页上限（缺省 200，不能超过代码上限 200）。登录响应带上这两项，列表下拉档不超过上限。请求未传 `pageSize` 时用租户默认值，超过上限时收成允许的最大值。默认仍是 20。网关日志查询的绑定上限同步为 200。

### 修复
- **集群端口右键能读到当前实例**：页面用表单提交的实例 ID 会参与探测。没有节点时，空状态在窗口中居中。
- **集群心跳没写上时不清理采集节点**：本轮登记失败就跳过清理，避免库短暂不可用时把 `HUB_METRIC_SERVER_INFO` 整表删掉。心跳更新碰到的行已被删掉时改为重新插入。
- **列表请求与后端分页字段对齐**：配置列表改为传 `pageIndex`。监控曲线固定查第一页，不跟服务器列表翻页走。
- **实例树都列出停用实例**：代理配置与路由配置的实例树都不再按启用状态过滤，停用实例仍可搜到。过滤器顺序相同时先把更大的顺序号后移再插入，避免撞号。导出超过 10 万条时直接失败，不返回半份数据。
- **选择器按页查询并标明总数**：服务定义、网关实例、过滤器不再一次装进下拉。本页之外的记录仍在库里，界面显示共多少条；搜索没有命中时写明没有匹配，加载失败不再显示成暂无。过滤器上移下移由服务端找相邻项。实例搜索同时匹配名称、地址和端口。过滤器统计和导出按全量计数，不再只算第一页。
- **进程退出不再回写服务中心实例状态**：`HUB_SERVICE_INSTANCE` 按租户、实例名、环境共用一行，没有 Pod 身份。滚动发布时旧 Pod 把这一行写成 `STOPPED` 或 `ERROR`，会盖住新 Pod 的 `RUNNING`。进程已进入停止流程时只停监听，不落库。管理面或集群事件主动停止时进程仍在运行，仍会写成 `STOPPED`。
- **进程退出时 context 取消不再记成 SQL 错误**：收到停止信号后，`appContext` 仍用来停后台循环。服务中心状态回写、集群、隧道和告警改走独立的 10 秒关闭上下文。只有进程已在停止且错误是 `context.Canceled` 时，数据库日志降为 info。清理超时、连接失败和 SQL 错误仍按 error 上报。
...
### 修复
- **Mongo 日志查询空结果不再把时间窗扫两遍**：附加条件不单独建索引。有命中时按时间倒序读满一页就停，所以多带几个字段仍然很快；一条都没有时会把该实例的时间窗读完，原先接着 `CountDocuments` 再读一遍。本页不满（含 0 条）时不再计数。
- **日志详情里报文和 Header 可以复制**：详情对话框中的代码块「复制」原先没有写入剪贴板。临时输入框改挂在当前对话框内，并在点击当下同步复制。只读代码块仍可选中正文。
- **启动脚本不再反复执行已存在的对象和数据**：表、列、索引、约束或种子数据已经在库里时，语句会报「已存在」。原先记成失败，下次启动再跑一遍，运维每次都能看到同一批错误。现在记为 `ALREADY_EXISTS`，之后直接跳过。真正失败的语句仍会重试。

## [3.4.1] - 2026-09-21

### 新增
- **环境设置可查看系统健康**：hub0009 增加「系统健康」页签，探测本进程已装配的平台库、ClickHouse、Mongo 与缓存。未加载的依赖记为未配置，不把整体打成异常。ClickHouse / Mongo 可看逻辑库名、系统表或集合、期望索引（含默认 `_id_`，倒序字段标 `desc`）是否就绪。失败只回分类文案，不回主机、账号或口令。进程探活仍走 `web.health.path`。

### 修复
- **升级后 Mongo 建索引不再卡住启动**：系统日志集合索引对齐改到后台（最长 2 小时），已有同名索引直接跳过，不再在听端口前同步 `CreateIndex`。百万级 `HUB_GW_ACCESS_LOG` 首次补齐仍可能较久，进程可以先起来。ClickHouse 建表失败仍只记警告，不挡启动。

## [3.4.0] - 2026-09-20

### 修复
- **管理端下线后 INTERNAL 不再转发该节点**：`UpdateNode` 先写本机再写活视图；叠 Redis 时若本机/事件的心跳更新，保留已下线的 `Status`，避免旧 UP 正文盖回。发现已无健康节点时清掉 `lastGood`，避免 45 秒兜底再打到已下线地址。
- **日志查询服务名称跟当前实例**：服务选择改走 `queryServiceDefinitions` 并带上 `gatewayInstanceId`，不再列出其它实例的服务。换实例时清空已选路由名和服务名。
...
### 修复
- **日志刷盘失败不再整批丢掉**：数据库 / Mongo / ClickHouse 定时 Flush 或满批直写遇到超时、选主、网络等可重试错误时，把该批插回缓冲头部，下次 Flush 优先重试。文档过大、重复键等数据错误仍丢弃，避免同一坏文档死循环。回灌条数上限为 `max(队列容量, 4 倍批量)`，到顶丢掉较新未刷条目。失败后按刷新间隔冷却（最短 1s），冷却期内满批只入缓冲、不再热循环 Insert。`Close` 先置 `closed` 再排空 Flush，关停后不再回灌。转发路径与主表字段不变。

### 变更
- **访问日志入队满则立刻丢**：`SubmitWriteLog` 改为 `OfferWait(0)`，不再短等 20ms。响应已写出且耗时已封口，空等会拖住 HTTP/1.1 复用连接上的下一条。队列仍 65536，满了丢弃并计数；worker、`WriteLog`、静态跳过不变。后端追踪仍满则短等 20ms。
- **日志查询条件对齐字段**：筛选「最小耗时」改名为「总处理时间」，仍打 `totalProcessingTimeMs`。高级条件新增「网关节点IP」，库 / Mongo / ClickHouse 对 `gatewayNodeIp` 精确匹配；列表同步展示该列。

## [3.3.6] - 2026-09-11

### 修复
- **密文传输在 HTTP 下可登录**：局域网 IP / 纯 HTTP 没有 `crypto.subtle`，包装改为回退 RSA-OAEP-SHA256，不再在 `importKey` 处抛错。
- **登录不再区分账号是否存在**：`/user/login` 在用户不存在与密码错误时返回同一文案和 `ED00103`。登录日志仍分别记「用户不存在 / 密码错误」。不存在的账号也会走一遍口令校验耗时，避免按快慢枚举。
- **个人资料不可越权改他人**：用户设置改为 `/user/profile`，身份只取 session。不再走 `hub0002/editUser`。请求里的 `userId` 忽略；只更新姓名/邮箱/手机/头像/性别，不能改部门、状态和管理员标记。`editUser` 仍需 `hub0002:edit`，只给用户管理。
- **登录验证码一次性核销**：图形验证码 HMAC 票在首次校验后立即作废（对错均核销）。时效内重放同一 `captchaId` 返回过期。本节点内存记录；有 default 缓存时用 `SetNX` 跨节点共享（切 Redis 后全集群生效）。

### 变更
- **环境设置 Web 访问支持密文传输**：原「Web访问超时」改为「Web访问」。可开启前后端口令字段 RSA-OAEP 包装；密钥对写入 `HUB_SYS_SETTING`（`webTimeout` 分组），私钥 `ENCY_` 入库，全集群共用，不写 `web.yaml`。关闭开关保留钥、不再轮换。登录、改密、建用户在开启后拒绝明文。
- **后端追踪离开返回路径**：`WriteBackendTraceLogSync` 在代理 defer 里只抽 ID、响应大小和已传入的头/体，入独立提交队列；组对象、头 JSON、body 截断和写库在 worker 中做。队列满同样短等 20ms，计入同一实例 pending，关写入器时与访问日志一起排空。从表字段、重试次数与多服务分别记账不变。
- **日志批处理超时生效**：`batchTimeoutMs` 不再只保存。数据库 / Mongo / ClickHouse 写库与文件刷盘用该值作为 I/O 超时。新建/补空默认 30000ms（对齐原先写超时）；已落库的旧表单 1000ms 运行时抬到 30000，不改库行。大于 1000 的已保存值照用。刷新间隔仍走 `asyncFlushIntervalMs`。建表 DDL 不变。
- **单服务请求体按需缓冲**：`proxyRequest` 不再无条件 `ReadAll`。会重试或记请求体时仍整包缓冲，`request_body` 为完整转发体，截断只在写库；两都不开则原样流向上游。多服务、主表字段与控制台重发不变。
- **转发头只深拷一次**：单服务 defer 里上下文和后端追踪共用一份发出头；多服务仍每条上游各拷一份只当参数传入，不写共享 ctx。主表 Original* 与从表字段不变。

## [3.3.5] - 2026-09-11

### 变更
- **HTTP 代理连接池默认值**：`MaxIdleConns` 默认由 100 调整为 2048（每 host 空闲约 512、每 host 上限约 4096）。已落库的实例仍用原配置，需在控制台改保存后生效。转发、重试与日志逻辑不变。
- **访问日志固定 worker 提交**：`finishRequest` 不再每条请求 `go WriteLog`。快照后入 65536 有界队列，由 GOMAXPROCS 个 worker（4～32）调用原 `WriteLog`（主表字段、端口重放 UPDATE、静态跳过不变）。队列满先等 20ms，仍满才丢弃并计数，不长时间反压转发。`GetAccessLogSubmitStats` 可看积压与丢弃数。
- **日志写入器对齐**：Mongo / 数据库 / ClickHouse / 文件共用有界入队（满则短等再丢）、队列/批量/刷新间隔走同一套默认值辅助函数、Flush 先拷缓冲再解锁写、关停先排空再刷剩余批、写库/刷盘 30s 超时。ClickHouse 批量下限 5000 只作用于写入器，不改共享配置；主表也尊重是否批量。文件写入器异步时不再在提交 worker 上持锁写盘，从表不再双重换行。控制台仍满则立刻丢，避免堵住 stdout。重放 UPDATE、reset、主表字段不变。
- **日志配置默认值对齐**：新建实例、控制台表单、保存补空与 `SetDefaults` 统一为异步队列 10000、刷新间隔 5000ms、批超时 5000ms；表单队列下限 100（与校验一致）。已落库行与建表 DDL 不变。
- **关停/重载不再提前拆写入器**：`CloseLogWriter` 先等该实例提交队列写完再关。同 ID 热更新等旧 writer 在途 `Write` 结束再 Close。实例 ID 是主键，重载不能改号；写入器只在新建 `InitLogManager`、删除 `CloseLogWriter`。
- **访问/追踪日志耗时**：毫秒按四舍五入，不足 1ms 的真实间隔记 1，避免打印成 0。主表总时间仍是墙钟；重试 `time.After` 等待从 `gatewayProcessingTimeMs` 中扣除，不再算成网关计算时间。表结构与 reset/重发不变。

## [3.3.4] - 2026-09-02

### 变更
- **网关实例配置改走数据库**：删除易误导的 `configs/gateway.yaml`。`app.gateway.configSource` 默认 `database`，监听端口与路由在控制台「网关管理」维护。yaml/json 仅作兼容路径，缺文件不再挡住启动。
- **日志控制台与行模板**：文件输出保持原逻辑。`output: console` 额外写标准输出供采集；`output: stdout` 与原先一样不生效。`encoding` 只控制 `json` / `console` 行格式。`format` 每种变量一个：`${time}` `${level}` `${msg}` `${caller}` `${stack}` `${logger}`。
- **进程级健康检查写在控制面配置**：`web.health.path`（默认 `/health`）挂在 `web.port`，不读实例表。实例配置损坏时数据面可能没起来，本接口仍返回 `ok`。
- **数据面监听口内置探活**：默认 `/_gw/health`，避开业务 `/health`。空/`off` 关闭且不包处理器。仅精确 `GET/HEAD`，在进引擎前返回。实例配置坏了听不上时没有这条口，不阻塞进程启动。
- **访问日志增加控制台输出**：日志配置「输出目标」可选 `CONSOLE`，写入进程标准输出供采集。后端原已支持，控制台表单补上该选项。

### 修复
- **数据面配置异常不再拖垮进程**：实例表查询失败、单条配置损坏、端口占用或证书错误只跳过对应实例并记日志，控制面（12003）继续启动。
- **控制台访问日志有界队列**：`ConsoleWriter` 单消费者写 stdout，`Write` 只投递；队列满丢弃并计数，避免 stdout 堵塞把快照堆在请求 goroutine 里。
...
### 修复
- **静态资源不再擅自 301**：未配置路由重定向时，静态托管不再给目录补尾斜杠。`sce-vcom-dialogs.sec.js` 这类地址不会再被改写成 `.js/` 后 404。目录请求直接出索引；带扩展名的 URL 落到目录时直接 404，不会把 index 当成脚本。
- **前缀匹配按路径段边界**：`/app` 只命中 `/app`、`/app/`、`/app/...`，不再误伤 `/application`、`/app.js`。`/app/*` 同样按段切；`/app*`（星号前无 `/`）仍按字面前缀。路径含 `/**` 的精确匹配按通配符处理子路径，兼容历史配置。
- **索引查找遵守符号链接策略**：目录索引改用 `Lstat`。未开跟随链接时，指向根外的目录链接回 403，不再经 `Stat` 读出站外 index。
- **检视错误不再当成缺失 404**：权限、符号链接、逃逸回 403；其它磁盘异常回 500，且不写入 5 秒缺失缓存。仅「文件不存在」才继续索引或 404。
- **静态响应记下真实状态码**：`ServeContent` 的 304 / 206 写入网关状态，访问日志不再一律记 200。成功命中（file/index/spa，含 304/206）仍不落库。

### 新增
- **静态托管可选目录补斜杠**：`redirectDirectorySlash` 默认关，打开后目录请求 301 到 `path/`（带扩展名的 URL 仍不补）。已有库由启动脚本对 `HUB_GW_STATIC_HOST_CONFIG.sql` 末尾 ALTER 增量补列，不必另跑 patch。
- **占位符精确匹配开关**：`rootTokenExact` 默认关（`d10app` 仍命中 `d10`）；打开后第一段必须整段相等。
- **OPTIONS 回 204**：静态路由不再对预检/探测回 405，便于已配 CORS 的跨域访问。
- **静态 ETag**：按修改时间与大小写出 `ETag`，支持 `If-None-Match`。
- **静态托管对齐常用 nginx 静态能力（分栏配置）**：备用目录（同一相对路径再找，最多 3 个）、按扩展名覆盖缓存、无预压缩时对文本现场 gzip、白名单页面安全头；HTML/文本自动 `charset=utf-8`。已有库由启动脚本对 `HUB_GW_STATIC_HOST_CONFIG.sql` 末尾 ALTER 增量补列。

### 变更
- **静态剥前缀与路由开关对齐**：路由 `stripPathPrefix` 为 Y 时，静态查找也会剥前缀。
- **静态结果码**：磁盘异常为 `error`（500）；可选目录 301 为 `redirect`；OPTIONS 为 `options`。路由后端「重定向」仍走 `proxy_type=redirect`。
- **Windows 连接点按链接处理**：未开跟随链接时与 symlink 一样拒绝。
- **缓存与 206 体积**：打开失败立刻踢掉缓存项；Range 响应按实际写出字节记账。
- **静态访问日志带最终路径**：异常访问日志备注增加 `static_path=`，为命中文件或最后一次查找的磁盘路径。

## [3.2.8] - 2026-08-21

### 变更
- **发版流水线 niuma-ui**：CI 钉到 npm `niuma-ui@1.1.7`（日期选择器年前后切换、输入框清除/密码按钮退出 Tab 序）。本地开发仍用 package.json 的 link。
- **登录响应不再返回 sessionId**：身份只走 HttpOnly Cookie `HUB_LG`。登录、`userinfo`、刷新会话的 JSON 不再带完整会话 ID。
- **租户管理员标记仅现任租户管理员可授**：`addUser` / `editUser` 忽略非管理员请求体中的 `tenantAdminFlag`；非管理员试图把 N 改成 Y 时 403。编辑时保留原值，避免表单默认 N 把管理员降权。
- **种子管理员首次登录须改密**：默认 `admin` / `123456` 仍可登录，但 `mustChangePwd=Y`，进系统前只能改密或登出。启动脚本按默认口令哈希增量 UPDATE，已改密的现网管理员不受影响。
- **改密特殊字符放宽、新建用户只校验长度**：改密仍要求大小写、数字和特殊字符，特殊字符改为任意标点或符号（不再限于 `@$!%*?&`）。新建用户初始口令只校验 8-20 位，首次登录再按完整策略改密。

### 修复
- **登录忘记密码 404**：无自助重置接口，改为说明页（联系管理员在用户管理中重置），并兼容旧路径 `/hub0001/forgot-password`。
- **顶栏用户菜单改为 Popover**：头像/姓名弹出卡片，主题切换放在弹层内，顶栏不再单独放主题按钮。

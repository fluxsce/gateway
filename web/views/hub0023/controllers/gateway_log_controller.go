...
// GetMonitoringOverview 获取网关监控概览数据（关系数据库版本）
// @Summary 获取网关监控概览数据（关系数据库版本）
...
// @Success 200 {object} response.JsonData
// @Router /gateway/hub0023/gateway-log/monitoring/overview [post]
func (c *GatewayLogController) GetMonitoringOverview(ctx *gin.Context) {
	// 解析查询参数
	...
	// 调用DAO查询
	overview, err := c.databaseMonitoringDAO.GetGatewayMonitoringOverview(ctx, &req)
	if err != nil {
...
// GetMonitoringChartData 获取网关监控图表数据（关系数据库版本）
// @Summary 获取网关监控图表数据（关系数据库版本）
...
// @Param query body models.GatewayMonitoringQueryRequest true "查询参数"
// @Success 200 {object} response.JsonData
// @Router /gateway/hub0023/gateway-log/monitoring/chart-data [post]
func (c *GatewayLogController) GetMonitoringChartData(ctx *gin.Context) {
	// 解析查询参数
	var req models.GatewayMonitoringQueryRequest
	if err := request.Bind(ctx, &req); err != nil {
	...
	// 并发查询各种监控数据
	requestTrend, err := c.databaseMonitoringDAO.GetRequestMetricsTrend(ctx, &req)
	if err != nil {
	...
	responseTimeTrend, err := c.databaseMonitoringDAO.GetResponseTimeMetricsTrend(ctx, &req)
	if err != nil {
	...
	statusCodeDistribution, err := c.databaseMonitoringDAO.GetStatusCodeDistribution(ctx, &req)
	if err != nil {
	...
	hotRoutes, err := c.databaseMonitoringDAO.GetHotRoutes(ctx, &req)
	if err != nil {
...
func (c *GatewayLogController) validateTimeRange(req *models.GatewayMonitoringQueryRequest) error {
	// 校验开始时间和结束时间是否为空
	if req.StartTime == "" {

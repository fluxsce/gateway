...
// BackendTraceLog 后端服务追踪日志模型
// 对应表结构：HUB_GW_BACKEND_TRACE_LOG
// 作为 HUB_GW_ACCESS_LOG 的从表，记录每个后端服务转发的详细信息
type BackendTraceLog struct {
	// 主键和关联字段
	TenantID       string `json:"tenantId" form:"tenantId" query:"tenantId" db:"tenantId"`                         // 租户ID（主键的一部分，确保租户隔离）
	TraceID        string `json:"traceId" form:"traceId" query:"traceId" db:"traceId"`                             // 链路追踪ID，关联主表 HUB_GW_ACCESS_LOG.traceId（主键的一部分）
	BackendTraceID string `json:"backendTraceId" form:"backendTraceId" query:"backendTraceId" db:"backendTraceId"` // 后端服务追踪ID（主键的一部分，用于区分同一请求的多个后端服务）

	// 服务信息（每个后端服务特定的）
	ServiceDefinitionID string `json:"serviceDefinitionId" form:"serviceDefinitionId" query:"serviceDefinitionId" db:"serviceDefinitionId"` // 服务定义ID
	ServiceName         string `json:"serviceName" form:"serviceName" query:"serviceName" db:"serviceName"`                                 // 服务名称（冗余字段，便于查询）

	// 转发信息（如果与主请求不同则记录，否则可为空）
	ForwardAddress string `json:"forwardAddress" form:"forwardAddress" query:"forwardAddress" db:"forwardAddress"` // 实际转发的目标地址（完整URL）
	ForwardMethod  string `json:"forwardMethod" form:"forwardMethod" query:"forwardMethod" db:"forwardMethod"`     // 转发的HTTP方法（GET, POST, PUT, DELETE等）
	ForwardPath    string `json:"forwardPath" form:"forwardPath" query:"forwardPath" db:"forwardPath"`             // 转发的路径（如果与主请求路径不同）
	ForwardQuery   string `json:"forwardQuery" form:"forwardQuery" query:"forwardQuery" db:"forwardQuery"`         // 转发的查询参数（如果与主请求不同）
	ForwardHeaders string `json:"forwardHeaders" form:"forwardHeaders" query:"forwardHeaders" db:"forwardHeaders"` // 转发的请求头（JSON格式，如果与主请求不同）
	ForwardBody    string `json:"forwardBody" form:"forwardBody" query:"forwardBody" db:"forwardBody"`             // 转发的请求体（如果与主请求不同）
	RequestSize    int    `json:"requestSize" form:"requestSize" query:"requestSize" db:"requestSize"`             // 请求大小（字节，向后端发送的请求体大小）

	// 负载均衡信息
	LoadBalancerStrategy string `json:"loadBalancerStrategy" form:"loadBalancerStrategy" query:"loadBalancerStrategy" db:"loadBalancerStrategy"` // 负载均衡策略（round-robin, random, weighted等）
	LoadBalancerDecision string `json:"loadBalancerDecision" form:"loadBalancerDecision" query:"loadBalancerDecision" db:"loadBalancerDecision"` // 负载均衡器的选择决策信息

	// 时间信息 - 精确到毫秒的时间戳（每个后端服务独立的）
	RequestStartTime     *time.Time `json:"requestStartTime" form:"requestStartTime" query:"requestStartTime" db:"requestStartTime"`                 // 向后端发起请求的时间（必填）
	ResponseReceivedTime *time.Time `json:"responseReceivedTime" form:"responseReceivedTime" query:"responseReceivedTime" db:"responseReceivedTime"` // 接收到后端响应的时间（零时间表示未收到响应或处理中）
	RequestDurationMs    int        `json:"requestDurationMs" form:"requestDurationMs" query:"requestDurationMs" db:"requestDurationMs"`             // 请求耗时（毫秒，0表示未完成）

	// 响应信息（每个后端服务独立的）
	StatusCode      int    `json:"statusCode" form:"statusCode" query:"statusCode" db:"statusCode"`                     // 后端服务返回的HTTP状态码（0表示未收到响应）
	ResponseSize    int    `json:"responseSize" form:"responseSize" query:"responseSize" db:"responseSize"`             // 响应大小（字节）
	ResponseHeaders string `json:"responseHeaders" form:"responseHeaders" query:"responseHeaders" db:"responseHeaders"` // 响应头信息（JSON格式，可选）
	ResponseBody    string `json:"responseBody" form:"responseBody" query:"responseBody" db:"responseBody"`             // 响应体内容（可选记录）

	// 错误信息（每个后端服务独立的）
	ErrorCode    string `json:"errorCode" form:"errorCode" query:"errorCode" db:"errorCode"`             // 错误代码
	ErrorMessage string `json:"errorMessage" form:"errorMessage" query:"errorMessage" db:"errorMessage"` // 详细错误信息

	// 状态信息
	SuccessFlag string `json:"successFlag" form:"successFlag" query:"successFlag" db:"successFlag"` // 是否成功（Y表示成功，N表示失败）
	TraceStatus string `json:"traceStatus" form:"traceStatus" query:"traceStatus" db:"traceStatus"` // 状态：pending（处理中）、success（成功）、failed（失败）、timeout（超时）
	RetryCount  int    `json:"retryCount" form:"retryCount" query:"retryCount" db:"retryCount"`     // 重试次数

	// 扩展信息
	ExtProperty string `json:"extProperty" form:"extProperty" query:"extProperty" db:"extProperty"` // 扩展属性（JSON格式，用于存储额外的服务特定信息）

	// 标准数据库字段
	AddTime        *time.Time `json:"addTime" form:"addTime" query:"addTime" db:"addTime"`                             // 记录创建时间
	AddWho         string     `json:"addWho" form:"addWho" query:"addWho" db:"addWho"`                                 // 记录创建者
	EditTime       *time.Time `json:"editTime" form:"editTime" query:"editTime" db:"editTime"`                         // 记录修改时间
	EditWho        string     `json:"editWho" form:"editWho" query:"editWho" db:"editWho"`                             // 记录修改者
	OprSeqFlag     string     `json:"oprSeqFlag" form:"oprSeqFlag" query:"oprSeqFlag" db:"oprSeqFlag"`                 // 操作序列标识
	CurrentVersion int        `json:"currentVersion" form:"currentVersion" query:"currentVersion" db:"currentVersion"` // 当前版本号
...
// TableName 指定表名
func (BackendTraceLog) TableName() string {
	return "HUB_GW_BACKEND_TRACE_LOG"
}

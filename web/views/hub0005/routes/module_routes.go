package hub0005routes

import (
	"gateway/pkg/database"
	"gateway/pkg/logger"
	"gateway/web/routes"
	"gateway/web/views/hub0005/controllers"
...
var (
	...
	// 实际路由时将根据RouteDiscovery的设置可能会使用"/api/hub0005"
	APIPrefix = "/gateway/hub0005"
)
...
// initRoleRoutes 初始化角色相关路由
// 将角色相关的所有API路由注册到指定的路由组
// 按RESTful风格组织API路径
//
// 参数:
//   - router: Gin路由组
//   - db: 数据库连接实例
func initRoleRoutes(router *gin.RouterGroup, db database.Database) {
	// 创建控制器
...
// 1. 在初始化阶段调用routes.RegisterModuleRoutes("hub0005", hub0005routes.RegisterRoutesFunc())

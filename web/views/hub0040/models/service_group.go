...
// ServiceGroup 服务分组模型，对应数据库HUB_REGISTRY_SERVICE_GROUP表
// 用于服务注册中心的命名空间管理
type ServiceGroup struct {
	// 主键和租户信息
	TenantId       string `json:"tenantId" form:"tenantId" query:"tenantId" db:"tenantId"`                         // 租户ID，联合主键
	ServiceGroupId string `json:"serviceGroupId" form:"serviceGroupId" query:"serviceGroupId" db:"serviceGroupId"` // 服务分组ID，联合主键
	GroupName      string `json:"groupName" form:"groupName" query:"groupName" db:"groupName"`                     // 分组名称，业务主键

	// 分组基本信息
	GroupDescription string `json:"groupDescription" form:"groupDescription" query:"groupDescription" db:"groupDescription"` // 分组描述
	GroupType        string `json:"groupType" form:"groupType" query:"groupType" db:"groupType"`                             // 分组类型(SYSTEM,BUSINESS,EXTERNAL)

	// 权限控制
	OwnerUserId          string `json:"ownerUserId" form:"ownerUserId" query:"ownerUserId" db:"ownerUserId"`                                     // 拥有者用户ID
	AdminUserIds         string `json:"adminUserIds" form:"adminUserIds" query:"adminUserIds" db:"adminUserIds"`                                 // 管理员用户ID列表，逗号分隔
	ReadUserIds          string `json:"readUserIds" form:"readUserIds" query:"readUserIds" db:"readUserIds"`                                     // 只读用户ID列表，逗号分隔
	AccessControlEnabled string `json:"accessControlEnabled" form:"accessControlEnabled" query:"accessControlEnabled" db:"accessControlEnabled"` // 是否启用访问控制(Y启用,N不启用)
	...
	DefaultHealthCheckIntervalSeconds int    `json:"defaultHealthCheckIntervalSeconds" form:"defaultHealthCheckIntervalSeconds" query:"defaultHealthCheckIntervalSeconds" db:"defaultHealthCheckIntervalSeconds"` // 默认健康检查间隔

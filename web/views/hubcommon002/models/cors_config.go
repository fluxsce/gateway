...
// CorsConfigQuery CORS配置查询条件，对应前端搜索表单的查询参数
type CorsConfigQuery struct {
	GatewayInstanceId string `json:"gatewayInstanceId" form:"gatewayInstanceId" query:"gatewayInstanceId"` // 网关实例ID（实例级CORS，与routeConfigId二选一）
	RouteConfigId     string `json:"routeConfigId" form:"routeConfigId" query:"routeConfigId"`             // 路由配置ID（路由级CORS，与gatewayInstanceId二选一）

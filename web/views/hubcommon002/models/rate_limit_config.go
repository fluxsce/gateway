...
// RateLimitConfigQuery 限流配置查询条件，对应前端搜索表单的查询参数
type RateLimitConfigQuery struct {
	GatewayInstanceId string `json:"gatewayInstanceId" form:"gatewayInstanceId" query:"gatewayInstanceId"` // 网关实例ID（实例级限流，与routeConfigId二选一）
	RouteConfigId     string `json:"routeConfigId" form:"routeConfigId" query:"routeConfigId"`             // 路由配置ID（路由级限流，与gatewayInstanceId二选一）
	LimitName         string `json:"limitName" form:"limitName" query:"limitName"`                         // 限流规则名称（模糊查询）
	Algorithm         string `json:"algorithm" form:"algorithm" query:"algorithm"`                         // 限流算法
	KeyStrategy       string `json:"keyStrategy" form:"keyStrategy" query:"keyStrategy"`                   // 限流键策略
}

...
// AuthConfigQuery 认证配置查询条件，对应前端搜索表单的查询参数
type AuthConfigQuery struct {
	GatewayInstanceId string `json:"gatewayInstanceId" form:"gatewayInstanceId" query:"gatewayInstanceId"` // 网关实例ID（实例级认证，与routeConfigId二选一）
	RouteConfigId     string `json:"routeConfigId" form:"routeConfigId" query:"routeConfigId"`             // 路由配置ID（路由级认证，与gatewayInstanceId二选一）
	AuthName          string `json:"authName" form:"authName" query:"authName"`                            // 认证配置名称（模糊查询）
}

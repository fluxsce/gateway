...
func (c *DomainAccessConfigController) AddDomainAccessConfig(ctx *gin.Context) {
	...
	// 查询最新的配置数据返回给前端
	newConfig, err := c.dao.GetDomainAccessConfigBySecurityConfigId(ctx, config.SecurityConfigId, tenantId)
	if err != nil {
	...
	logger.InfoWithTrace(ctx, "域名访问控制配置添加成功", "securityConfigId", config.SecurityConfigId,
...
func (c *DomainAccessConfigController) GetDomainAccessConfig(ctx *gin.Context) {
	...
	// 调用DAO层获取域名访问控制配置
	config, err := c.dao.GetDomainAccessConfigBySecurityConfigId(ctx, securityConfigId, tenantId)
	if err != nil {
	...
	logger.InfoWithTrace(ctx, "获取域名访问控制配置成功", "securityConfigId", securityConfigId, "tenantId", tenantId)
...
func (c *DomainAccessConfigController) UpdateDomainAccessConfig(ctx *gin.Context) {
	...
	// 查询最新的配置数据返回给前端
	updatedConfig, err := c.dao.GetDomainAccessConfigBySecurityConfigId(ctx, securityConfigId, tenantId)
	if err != nil {
	...
	logger.InfoWithTrace(ctx, "域名访问控制配置更新成功", "securityConfigId", securityConfigId,
		"tenantId", tenantId, "operatorId", operatorId)
...
func (c *DomainAccessConfigController) DeleteDomainAccessConfig(ctx *gin.Context) {
	...
	// 调用DAO层删除域名访问控制配置
	err := c.dao.DeleteDomainAccessConfig(ctx, securityConfigId, tenantId, operatorId)
	if err != nil {
	...
	logger.InfoWithTrace(ctx, "域名访问控制配置删除成功", "securityConfigId", securityConfigId,
		"tenantId", tenantId, "operatorId", operatorId)
...
func (c *DomainAccessConfigController) QueryDomainAccessConfigs(ctx *gin.Context) {
	...
	// 调用DAO层查询域名访问控制配置列表
	configs, total, err := c.dao.ListDomainAccessConfigs(ctx, tenantId, page, pageSize)
	if err != nil {
	...
	logger.InfoWithTrace(ctx, "查询域名访问控制配置列表成功", "tenantId", tenantId, "count", len(configs))

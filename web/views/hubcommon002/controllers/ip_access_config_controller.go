...
func (c *IpAccessConfigController) AddIpAccessConfig(ctx *gin.Context) {
	...
	// 查询最新的配置数据返回给前端
	newConfig, err := c.dao.GetIpAccessConfigBySecurityConfigId(ctx, config.SecurityConfigId, tenantId)
	if err != nil {
...
func (c *IpAccessConfigController) GetIpAccessConfig(ctx *gin.Context) {
	...
	// 调用DAO层获取IP访问控制配置
	config, err := c.dao.GetIpAccessConfigBySecurityConfigId(ctx, securityConfigId, tenantId)
	if err != nil {
	...
	logger.InfoWithTrace(ctx, "获取IP访问控制配置成功", "securityConfigId", securityConfigId, "tenantId", tenantId)
...
func (c *IpAccessConfigController) UpdateIpAccessConfig(ctx *gin.Context) {
	...
	// 查询最新的配置数据返回给前端
	updatedConfig, err := c.dao.GetIpAccessConfigBySecurityConfigId(ctx, securityConfigId, tenantId)
	if err != nil {
	...
	logger.InfoWithTrace(ctx, "IP访问控制配置更新成功", "securityConfigId", securityConfigId, 
		"tenantId", tenantId, "operatorId", operatorId)
...
func (c *IpAccessConfigController) DeleteIpAccessConfig(ctx *gin.Context) {
	...
	// 调用DAO层删除IP访问控制配置
	err := c.dao.DeleteIpAccessConfig(ctx, securityConfigId, tenantId, operatorId)
	if err != nil {
	...
	logger.InfoWithTrace(ctx, "IP访问控制配置删除成功", "securityConfigId", securityConfigId, 
		"tenantId", tenantId, "operatorId", operatorId)
...
func (c *IpAccessConfigController) QueryIpAccessConfigs(ctx *gin.Context) {
	...
	// 调用DAO层查询IP访问控制配置列表
	configs, total, err := c.dao.ListIpAccessConfigs(ctx, tenantId, page, pageSize)
	if err != nil {
	...
	logger.InfoWithTrace(ctx, "查询IP访问控制配置列表成功", "tenantId", tenantId, "count", len(configs))

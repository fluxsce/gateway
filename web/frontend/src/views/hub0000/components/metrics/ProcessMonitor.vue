<template>
    <RsCard :title="t('hub0000.process.title')" :padding="false" borderless variant="plain" class="monitor-card">
        <template #actions>
    ...
    const option = {
        tooltip: createAxisTooltipOptions({
            ...
            formatter: (params: any) => {
                ...
                // 添加前5个进程的CPU和内存使用率
                if (processes.length > 0) {
                    result += '<br/><b>' + t('hub0000.process.detailTitle') + '</b><br/>'
        ...
        legend: {
            data: [t('hub0000.process.avgCpu'), t('hub0000.process.avgMemory')],
            bottom: 0,
            padding: [10, 20]
        },
        grid: {
        ...
        series: [
            ...
            {
                name: t('hub0000.process.avgMemory'),
                type: 'line',
                data: memoryData,
                smooth: true,
                showSymbol: false,

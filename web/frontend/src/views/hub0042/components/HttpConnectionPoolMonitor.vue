<template>
  <div class="http-connection-pool-monitor">
    <!-- 控制栏 -->
...
const loadData = async () => {
  ...
  try {
    ...
    if (isApiSuccess(result)) {
      ...
      chartData.value = allData.filter(d => d.dataType === 'HTTP_CONNECTION_POOL')
      
      // 更新连接池名称列表（只在首次加载或没有选中时更新）
    ...
    console.error('加载HTTP连接池数据失败:', error)
  } finally {
    loading.value = false
  }
}
...
const renderChart = () => {
  ...
  const option: echarts.EChartsOption = {
        ...
        return `
          <div style="font-size: 12px;">
            ...
            <div style="color: #666; margin-top: 8px; padding-top: 8px; border-top: 1px solid #e8e8e8;">
              <div style="margin: 3px 0;">${t('leased')}: ${data.totalLeased || 0}</div>
              <div style="margin: 3px 0;">${t('available')}: ${data.totalAvailable || 0}</div>
              <div style="margin: 3px 0;">${t('pending')}: ${data.totalPending || 0}</div>
              <div style="margin: 3px 0;">最大连接数: ${data.totalMax || 0}</div>
              <div style="margin: 3px 0;">总连接数: ${data.totalConnections || 0}</div>
              <div style="margin: 3px 0;">使用率: ${((data.connectionUsageRatio || 0) * 100).toFixed(2)}%</div>
            </div>
          </div>
        `
      }
    ...
    legend: {
      data: [t('leased'), t('available'), t('pending')],
      bottom: 0
    },
    grid: {
      left: '3%',
    ...
    series: [
      {
        ...
        data: rawData.map(d => d.totalLeased || 0)
      },
      {
        name: t('available'),
        type: 'line',
        ...
        data: rawData.map(d => d.totalAvailable || 0)
      },
      {
        name: t('pending'),
        type: 'line',
        ...
        itemStyle: {
          color: (params: any) => {
            const item = poolData[params.dataIndex]
            return item.healthyFlag === 'N' ? '#f5222d' : '#f5222d'
          }
        },
        data: rawData.map(d => d.totalPending || 0)
      }
    ]
  }

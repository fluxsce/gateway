<template>
  <RsDialog
    :open="modalVisible"
    :title="props.title || 'User-Agent访问控制配置列表'"
    layout="window"
  ...
  >
    <template #body>
      <div class="useragent-access-config-list-modal" id="useragent-access-config-list-modal">
        <RsSplitPane
          class="useragent-access-config-list-modal__split"
          orientation="vertical"
          :panes="splitPanes"
          disabled
        >
          <template #search>
            <div class="useragent-access-config-list-modal__search">
              <RsSearchForm
                ref="searchFormRef"
          ...
          <template #grid>
            <div class="useragent-access-config-list-modal__grid">
              <RsGrid
                ref="gridRef"
                :module-id="service.model.moduleId"
...
import { useUserAgentAccessConfigPage } from './hooks'
import type { UserAgentAccessConfigListModalEmits, UserAgentAccessConfigListModalProps } from './hooks/types'

defineOptions({
  name: 'UserAgentAccessConfigListModal',
})

const props = withDefaults(defineProps<UserAgentAccessConfigListModalProps>(), {
  visible: false,
  title: 'User-Agent访问控制配置列表',
  width: 1200,
  to: undefined,
  securityConfigId: undefined,
})

const emit = defineEmits<UserAgentAccessConfigListModalEmits>()
...
<style scoped>
.useragent-access-config-list-modal {
...
.useragent-access-config-list-modal__split {
  flex: 1;
  min-height: 0;
  height: 100%;
}

.useragent-access-config-list-modal__search {
  width: 100%;
}

.useragent-access-config-list-modal__grid {
  height: 100%;
  min-height: 0;
  overflow: hidden;
  display: flex;

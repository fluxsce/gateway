 ...
 * - 组合 useUserAgentAccessConfigService（纯业务逻辑）
...
import { useUserAgentAccessConfigService } from './useUserAgentAccessConfigService'
...
export function useUserAgentAccessConfigPage(
  ...
  const currentEditConfig = ref<UserAgentAccessConfig | null>(null)

  /**
   * 将配置对象转换为表单数据格式（解析 JSON 字符串字段为数组）
   */
  const convertToFormData = (config: UserAgentAccessConfig): any => {
    return {
      ...config,
      whitelistPatterns: parseArrayField(config.whitelistPatterns as any),
      blacklistPatterns: parseArrayField(config.blacklistPatterns as any),
    }
  }
  ...
  /** 打开编辑配置对话框 */
  const openEditDialog = async (config: UserAgentAccessConfig) => {
    // 如果有 useragentAccessConfigId，通过主键获取最新数据
    if (config.useragentAccessConfigId) {
      try {
        const latestConfig = await service.getConfigDetail(config.useragentAccessConfigId)
        if (latestConfig) {
  ...
  /** 打开查看详情对话框 */
  const openViewDialog = async (config: UserAgentAccessConfig) => {
    // 如果有 useragentAccessConfigId，通过主键获取最新数据
    if (config.useragentAccessConfigId) {
      try {
        const latestConfig = await service.getConfigDetail(config.useragentAccessConfigId)
  ...
  const handleFormSubmit = async (formData?: Record<string, any>) => {
    ...
    const apiData: any = {
      ...formData,
      whitelistPatterns: Array.isArray(formData.whitelistPatterns) ? JSON.stringify(formData.whitelistPatterns) : formData.whitelistPatterns,
      blacklistPatterns: Array.isArray(formData.blacklistPatterns) ? JSON.stringify(formData.blacklistPatterns) : formData.blacklistPatterns,
    }
    ...
    if (formDialogMode.value === 'create') {
      ...
      const success = await service.addConfig(apiData as Partial<UserAgentAccessConfig> & { securityConfigId: string })
      if (success) {
        closeFormDialog()
      }
      ...
      const updatedConfig = {
        ...
        ...apiData,
      } as Partial<UserAgentAccessConfig> & { securityConfigId: string; useragentAccessConfigId: string; tenantId: string }
  ...
  const handleToolbarClick = async (key: string, formData?: Record<string, any>) => {
    switch (key) {
      ...
      case 'edit': {
        ...
        openEditDialog(currentRow as UserAgentAccessConfig)
        break
      }
      ...
      case 'delete': {
        ...
        await service.deleteConfig(currentRow as UserAgentAccessConfig)
        break
      }
   ...
   * 右键菜单点击处理
   */
  const handleMenuClick = async ({ code, row }: { code: string; row?: UserAgentAccessConfig }) => {
    if (!row) return

/**
 * IP访问控制配置列表页面级 Hook
 * - 组合 useIpAccessConfigService（纯业务逻辑）
...
import { useIpAccessConfigService } from './useIpAccessConfigService'
 ...
 */
export function useIpAccessConfigPage(
  gridRef?: Ref<any> | any,
  ...
  // 业务服务（包含 model、增删改查等）
  const service = useIpAccessConfigService(securityConfigId, searchFormRef)

  // 表单对话框状态（新增/编辑/查看共用）
  const formDialogVisible = ref(false)
  const formDialogMode = ref<'create' | 'edit' | 'view'>('create')
  const currentEditConfig = ref<IpAccessConfig | null>(null)

  /**
   * 将配置对象转换为表单数据格式（解析 JSON 字符串字段为数组）
   */
  const convertToFormData = (config: IpAccessConfig): any => {
    return {
      ...config,
      whitelistIps: parseArrayField(config.whitelistIps as any),
      blacklistIps: parseArrayField(config.blacklistIps as any),
      whitelistCidrs: parseArrayField(config.whitelistCidrs as any),
      blacklistCidrs: parseArrayField(config.blacklistCidrs as any),
    }
  }
  ...
  /** 打开编辑配置对话框 */
  const openEditDialog = async (config: IpAccessConfig) => {
    // 如果有 ipAccessConfigId，通过主键获取最新数据
    if (config.ipAccessConfigId) {
      try {
        const latestConfig = await service.getConfigDetail(config.ipAccessConfigId)
        if (latestConfig) {
  ...
  /** 打开查看详情对话框 */
  const openViewDialog = async (config: IpAccessConfig) => {
    // 如果有 ipAccessConfigId，通过主键获取最新数据
    if (config.ipAccessConfigId) {
      try {
        const latestConfig = await service.getConfigDetail(config.ipAccessConfigId)
  ...
  const handleFormSubmit = async (formData?: Record<string, any>) => {
    ...
    const apiData: any = {
      ...formData,
      whitelistIps: Array.isArray(formData.whitelistIps) ? JSON.stringify(formData.whitelistIps) : formData.whitelistIps,
      blacklistIps: Array.isArray(formData.blacklistIps) ? JSON.stringify(formData.blacklistIps) : formData.blacklistIps,
      whitelistCidrs: Array.isArray(formData.whitelistCidrs) ? JSON.stringify(formData.whitelistCidrs) : formData.whitelistCidrs,
      blacklistCidrs: Array.isArray(formData.blacklistCidrs) ? JSON.stringify(formData.blacklistCidrs) : formData.blacklistCidrs,
    }
    ...
    if (formDialogMode.value === 'create') {
      ...
      const success = await service.addConfig(apiData as Partial<IpAccessConfig> & { securityConfigId: string })
      if (success) {
        closeFormDialog()
      }
      ...
      const updatedConfig = {
        ...
        ...apiData,
      } as Partial<IpAccessConfig> & { securityConfigId: string; ipAccessConfigId: string; tenantId: string }
  ...
  const handleToolbarClick = async (key: string, formData?: Record<string, any>) => {
    switch (key) {
      ...
      case 'edit': {
        ...
        openEditDialog(currentRow as IpAccessConfig)
        break
      }
      ...
      case 'delete': {
        ...
        await service.deleteConfig(currentRow as IpAccessConfig)
        break
      }
...
export type IpAccessConfigPage = ReturnType<typeof useIpAccessConfigPage>

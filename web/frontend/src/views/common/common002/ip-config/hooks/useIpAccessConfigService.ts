...
import { useIpAccessConfigModel } from './model'
...
// 初始化 Model（传递 moduleId）
  ...
  const loadConfigs = async (searchParams?: Record<string, any>) => {
    ...
    try {
      ...
      const response: JsonDataObj = await securityApi.queryIpAccessConfigs(params)

      if (isApiSuccess(response)) {
        // 解析业务数据
        const configs = parseJsonData<IpAccessConfig[]>(response, [])
        ...
        message.error(getApiMessage(response, '查询IP访问控制配置列表失败'))
      }
    } catch (error) {
      message.error('加载IP访问控制配置列表失败')
    } finally {
      loading.value = false
    }
  }
   ...
   * 添加IP配置
   */
  const addConfig = async (configData: Partial<IpAccessConfig> & { securityConfigId: string }): Promise<boolean> => {
    loading.value = true
    try {
      const response: JsonDataObj = await securityApi.addIpAccessConfig(configData)

      if (isApiSuccess(response)) {
        message.success(getApiMessage(response, '新增IP访问控制配置成功'))
        
        // 如果返回了新增的配置数据，添加到列表
        const newConfig = parseJsonData<IpAccessConfig | null>(response, null)
        if (newConfig) {
        ...
        message.error(getApiMessage(response, '新增IP访问控制配置失败'))
        return false
      }
    } catch (error) {
      message.error('新增IP访问控制配置失败')
      return false
    } finally {
      loading.value = false
    }
   ...
   * 编辑IP配置
   */
  const editConfig = async (configData: Partial<IpAccessConfig> & { securityConfigId: string; ipAccessConfigId: string; tenantId: string }): Promise<boolean> => {
    loading.value = true
    try {
      const response: JsonDataObj = await securityApi.updateIpAccessConfig(configData)

      if (isApiSuccess(response)) {
        message.success(getApiMessage(response, '编辑IP访问控制配置成功'))
        
        // 更新列表中的配置数据：必须以返回的 response.bizData 为准
        const updatedConfig = parseJsonData<IpAccessConfig | null>(response, null)
        if (updatedConfig) {
          updateConfigInList(updatedConfig.ipAccessConfigId, updatedConfig.tenantId, updatedConfig)
        } else {
        ...
        message.error(getApiMessage(response, '编辑IP访问控制配置失败'))
        return false
      }
    } catch (error) {
      message.error('编辑IP访问控制配置失败')
      return false
    } finally {
      loading.value = false
    }
   ...
   * 删除IP配置
   */
  const deleteConfig = async (config: IpAccessConfig): Promise<boolean> => {
    const confirmed = await rsConfirm.warning({
      title: '确认删除',
      subtitle: '此操作不可恢复，请谨慎操作',
      description: `确定要删除IP访问控制配置 "${config.configName}" 吗？`,
      icon: WarningOutline,
      confirmText: '确定删除',
      cancelText: '取消',
      width: 500
    ...
    loading.value = true
    try {
      const response: JsonDataObj = await securityApi.deleteIpAccessConfig({
        ipAccessConfigId: config.ipAccessConfigId,
        tenantId: config.tenantId
      })

      if (isApiSuccess(response)) {
        message.success(getApiMessage(response, '删除IP访问控制配置成功'))
        ...
        message.error(getApiMessage(response, '删除IP访问控制配置失败'))
        return false
      }
    } catch (error) {
      message.error('删除IP访问控制配置失败')
      return false
    } finally {
      loading.value = false
    }
  ...
  /**
   * 获取配置详情（使用主键 ipAccessConfigId）
   */
  const getConfigDetail = async (ipAccessConfigId: string): Promise<IpAccessConfig | null> => {
    try {
      const response: JsonDataObj = await securityApi.getIpAccessConfig({
        ipAccessConfigId
      })
      
      if (isApiSuccess(response)) {
        return parseJsonData<IpAccessConfig>(response, {} as IpAccessConfig)
      } else {
        message.error(getApiMessage(response, '获取IP访问控制配置详情失败'))
        return null
      }
    } catch (error) {
      message.error('获取IP访问控制配置详情失败')
      return null
    }
  }

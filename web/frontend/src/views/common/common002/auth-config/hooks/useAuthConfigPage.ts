...
export function useAuthConfigPage(
  ...
  /**
   * 将配置对象转换为表单数据格式（查询时：将 authConfig JSON 字符串解析为嵌套对象）
   */
  const convertToFormData = (config: AuthConfig): any => {
    ...
    const formData: any = {
      ...
      authConfig: authConfigObj && typeof authConfigObj === 'object' ? { ...authConfigObj } : {},
    }

    if (config.authType === 'API_KEY') {
      normalizeApiKeyFormFields(formData, authConfigObj && typeof authConfigObj === 'object' ? authConfigObj : {})
    }
  ...
  /**
   * 将表单数据转换为API数据格式（保存时：将 authConfig 嵌套对象合并为一个 JSON 字符串）
   */
  const convertToApiData = (formData: Record<string, any>): any => {
    ...
    // 构建 API 数据对象，排除 authConfig 子树（稍后单独 stringify）
    const apiData: Record<string, any> = {}
    Object.keys(formData).forEach((key) => {
      if (key === 'authConfig' || key.startsWith('authConfig.')) return
  ...
  const handleFormSubmit = async (formData?: Record<string, any>) => {
    ...
    // 将表单数据转换为API数据格式（嵌套对象转为 JSON 字符串）
    ...
    if (formDialogMode.value === 'create') {
      ...
      // 注意：apiData 已经在组件层面完成了数据转换（嵌套对象合并、JSON 转换等）
      const updatedConfig = {
        ...apiData,

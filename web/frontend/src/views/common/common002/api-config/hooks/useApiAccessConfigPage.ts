/**
 * API访问控制配置列表页面级 Hook
 * - 组合 useApiAccessConfigService（纯业务逻辑）
...
import { useApiAccessConfigService } from './useApiAccessConfigService'
 ...
 */
export function useApiAccessConfigPage(
  moduleId: Ref<string>,
  gridRef?: Ref<any> | any,
  ...
  const service = useApiAccessConfigService(moduleId.value, securityConfigId, searchFormRef)

  // 表单对话框状态（新增/编辑/查看共用）
  const formDialogVisible = ref(false)
  const formDialogMode = ref<'create' | 'edit' | 'view'>('create')
  const currentEditConfig = ref<ApiAccessConfig | null>(null)

  /**
   * 将配置对象转换为表单数据格式（解析 JSON 字符串字段为数组）
   */
  const convertToFormData = (config: ApiAccessConfig): any => {
    return {
      ...config,
      whitelistPaths: parseArrayField(config.whitelistPaths as any),
      blacklistPaths: parseArrayField(config.blacklistPaths as any),
      allowedMethods: parseArrayField(config.allowedMethods as any),
      blockedMethods: parseArrayField(config.blockedMethods as any),
    }
  }
  ...
  /** 打开编辑配置对话框 */
  const openEditDialog = async (config: ApiAccessConfig) => {
    // 如果有 apiAccessConfigId，通过主键获取最新数据
    if (config.apiAccessConfigId) {
      try {
        const latestConfig = await service.getConfigDetail(config.apiAccessConfigId)
        if (latestConfig) {
  ...
  /** 打开查看详情对话框 */
  const openViewDialog = async (config: ApiAccessConfig) => {
    // 如果有 apiAccessConfigId，通过主键获取最新数据
    if (config.apiAccessConfigId) {
      try {
        const latestConfig = await service.getConfigDetail(config.apiAccessConfigId)
  ...
  const handleFormSubmit = async (formData?: Record<string, any>) => {
    ...
    const apiData: any = {
      ...formData,
      whitelistPaths: Array.isArray(formData.whitelistPaths) ? JSON.stringify(formData.whitelistPaths) : formData.whitelistPaths,
      blacklistPaths: Array.isArray(formData.blacklistPaths) ? JSON.stringify(formData.blacklistPaths) : formData.blacklistPaths,
      allowedMethods: Array.isArray(formData.allowedMethods) ? JSON.stringify(formData.allowedMethods) : formData.allowedMethods,
      blockedMethods: Array.isArray(formData.blockedMethods) ? JSON.stringify(formData.blockedMethods) : formData.blockedMethods,
    }
    ...
    if (formDialogMode.value === 'create') {
      ...
      const success = await service.addConfig(apiData as Partial<ApiAccessConfig> & { securityConfigId: string })
      if (success) {
        closeFormDialog()
      }
      ...
      const updatedConfig = {
        ...
        ...apiData,
      } as Partial<ApiAccessConfig> & { securityConfigId: string; apiAccessConfigId: string; tenantId: string }
  ...
  const handleToolbarClick = async (key: string, formData?: Record<string, any>) => {
    switch (key) {
      ...
      case 'edit': {
        ...
        openEditDialog(currentRow as ApiAccessConfig)
        break
      }
      ...
      case 'delete': {
        ...
        await service.deleteConfig(currentRow as ApiAccessConfig)
        break
      }
   ...
   * 右键菜单点击处理
   */
  const handleMenuClick = async ({ code, row }: { code: string; row?: ApiAccessConfig }) => {
    if (!row) return

...
import { useGatewayInstanceModel } from './model'
...
export function useGatewayInstanceService(searchFormRef?: Ref<any> | any) {
   ...
   * 添加实例
   */
  const addInstance = async (instanceData: GatewayInstance): Promise<boolean> => {
    loading.value = true
    try {
      // 转换表单数据，将TLS版本数组转换为逗号分隔的字符串
      const formData = {
        ...instanceData,
        tlsVersion: Array.isArray(instanceData.tlsVersion)
          ? instanceData.tlsVersion.join(',')
          : instanceData.tlsVersion,
      } as any

      // 移除日志配置数据，日志配置通过单独的API处理
      delete formData.logConfig

      const response: JsonDataObj = await gatewayApi.addGatewayInstance(formData)

      if (isApiSuccess(response)) {
        const successMsg = getApiMessage(response, '网关实例创建成功')
   ...
   * 编辑实例
   */
  const editInstance = async (instanceData: Partial<GatewayInstance> & {
    ...
    try {
      ...
      const response: JsonDataObj = await gatewayApi.editGatewayInstance(formData)

      if (isApiSuccess(response)) {
        const successMsg = getApiMessage(response, '网关实例更新成功')
        message.success(successMsg)
        
        // 更新列表中的实例数据
        if (response.bizData) {
          const updatedInstance = JSON.parse(response.bizData)
          updateInstanceInList(updatedInstance.gatewayInstanceId, updatedInstance.tenantId, updatedInstance)
        } else {
          // 如果没有返回数据，使用提交的数据更新
          const existingInstance = instanceList.value.find(
            (i) => i.gatewayInstanceId === instanceData.gatewayInstanceId
          )
          if (existingInstance) {
            updateInstanceInList(instanceData.gatewayInstanceId, existingInstance.tenantId, formData)
          }
        }
  ...
  const deleteInstance = async (instance: GatewayInstance): Promise<boolean> => {
    ...
    loading.value = true
    try {
      const response: JsonDataObj = await gatewayApi.deleteGatewayInstance(
   ...
   * 启动实例
   */
  const startInstance = async (instance: GatewayInstance): Promise<boolean> => {
    loading.value = true
    try {
      const response: JsonDataObj = await gatewayApi.startGatewayInstance(
        instance.gatewayInstanceId,
        instance.tenantId
      )
   ...
   * 停止实例
   */
  const stopInstance = async (instance: GatewayInstance): Promise<boolean> => {
    loading.value = true
    try {
      const response: JsonDataObj = await gatewayApi.stopGatewayInstance(
        instance.gatewayInstanceId,
        instance.tenantId
      )
   ...
   * 重新加载实例配置
   */
  const reloadInstance = async (instance: GatewayInstance): Promise<boolean> => {
    loading.value = true
    try {
      const response: JsonDataObj = await gatewayApi.reloadGatewayInstance(
        instance.gatewayInstanceId,
        instance.tenantId
      )

      if (isApiSuccess(response)) {
        const successMsg = getApiMessage(response, '网关重载成功')
        message.success(successMsg)
        // 重新加载列表以获取最新状态
        await loadInstances()
        return true
   ...
   * 获取实例详情
   */
  const getInstanceDetail = async (gatewayInstanceId: string, tenantId: string): Promise<GatewayInstance | null> => {
    try {
      const response: JsonDataObj = await gatewayApi.getGatewayInstance(gatewayInstanceId, tenantId)
      if (isApiSuccess(response)) {
        const instance = parseJsonData<GatewayInstance | null>(response, null)
        return instance
      } else {
 ...
 * 网关实例服务类型
 */
export type GatewayInstanceService = ReturnType<typeof useGatewayInstanceService>
